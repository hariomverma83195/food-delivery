...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        // Use the matchPassword method instead of bcrypt.compare()
...
app.post("/login/restaurant", async (req, res) => {
  ...
  if (!user) {
   ...
   res.json({ success: true, dashboard: "/dashboard/restaurant" });
});
...
app.post("/login/deliverypartner", async (req, res) => {
  const { email, password } = req.body;
  const user = await Restaurant.findOne({ email });
  const isMatch = await bcrypt.compare(password, user.deliverypartnerpass);
  if (!isMatch) {
       return res.json({ success: false, message: "Invalid credentials" });
  }
...
app.post("/restaurant/update-delivery-password", async (req, res) => {
    ...
    try {
        await Restaurant.findByIdAndUpdate(restaurantId, { deliverypartnerpass: password });
